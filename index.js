// boots up the app
const apikey = "171cce76de0c96e7397d1dcba05dbb71";
...
const apiPaths = {
    ...
    searchOnYoutube: (query) => `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query}&key=AIzaSyBRmI7HDLqt6uY31Ej6Vw-hptYjWw_ZOHY`
}
...
async function fetchTrendingMovies() {
  try {
    const response = await fetch(apiPaths.fetchTrending);
    const data = await response.json();
    const movies = data.results;
    if (Array.isArray(movies) && movies.length) {
      const randomIndex = Math.floor(Math.random() * movies.length);
      buildBannerSection(movies[randomIndex]);
    }
  } catch (err) {
    console.error(err);
  }
...
async function fetchAndBuildAllSections() {
  try {
    const response = await fetch(apiPaths.fetchAllCategories);
    const data = await response.json();
...
async function searchMovieTrailer(movieName, iframeId) {
  if (!movieName) return;

  try {
    const response = await fetch(apiPaths.searchOnYoutube(movieName));
    const data = await response.json();
